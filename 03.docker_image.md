...
### DockerFile 
: 손쉽게 기록하고 수행할 수 있는 빌드 명령어를 제공하여  
패키지, 소스코드, 셸스크립트등을 파일에 기록해두면 도커는 이파일을 읽음  
이러한 작업을 기록한 파일의 이름을 Dockerfile이며,  
빌드 명령어는 Dockerfile을 읽어서 이미지를 생성합니다.
Dockerfile을 사용하면 직접 컨테이너를 생성하고 커밋하는 번거로움은 있지만  
깃과같은 개발도구를 통해 애플리케이션의 빌드 및 배포를 자동화할수있다.
